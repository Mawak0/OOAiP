...
public class AuthCommand : ICommand
{
    ...
    public void Execute()
    {
        var legalOrdersRepository = Ioc.Resolve<Dictionary<string, Dictionary<string, string[]>>>("LegalOrder.Repository");
        bool flagCommandCheck = false;

        if (legalOrdersRepository.ContainsKey(user) && legalOrdersRepository[user].ContainsKey(command))
        {
            legalOrdersRepository[user].TryGetValue(command, out string[]? objects);
